...
public class MainExceptionHandlers {
    ...
    @ExceptionHandler({NoFoundException.class, FoundException.class, NoAccessOperation.class})
    ...
    public ResponseEntity<?> run(RuntimeException e){
        log.error(e.getMessage());
        return ResponseEntity.ok(ResponseDto.builder().error("Что-то произошло. Попробуйте позже").build());
    }

    @ExceptionHandler({Exception.class})
    public ResponseEntity<?> main(Exception e){
        log.error(e.getMessage());
        return ResponseEntity.ok(ResponseDto.builder().error("Похоже что-то серьёзное, попробуйте позже").build());
    }
}
